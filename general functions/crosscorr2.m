function varargout = crosscorr2(Series1 , Series2 , nLags , nSTDs)
...
%   $Revision: 1.6.2.2 $  $Date: 2007/09/11 11:45:57 $

if nargin < 2
   error('econ:crosscorr:UnspecifiedInput' , ' ''Series1'' and ''Series2'' are both required.');
end
...
if ((rows ~= 1) && (columns ~= 1)) || (rows*columns < 2)
   error('econ:crosscorr:NonVectorSeries1' , ' Input ''Series1'' must be a vector.');
end

[rows , columns]  =  size(Series2);

if ((rows ~= 1) && (columns ~= 1)) || (rows*columns < 2)
   error('econ:crosscorr:NonVectorSeries2' , ' Input ''Series2'' must be a vector.');
else
...
if (nargin >= 3) && ~isempty(nLags)
   if numel(nLags) > 1
      error('econ:crosscorr:NonScalarLags' , ' Number of lags ''nLags'' must be a scalar.');
   end
   if (round(nLags) ~= nLags) || (nLags <= 0)
      error('econ:crosscorr:NonPositiveInteger' , ' Number of lags ''nLags'' must be a positive integer.');
   end
   if nLags > (n - 1)
      error('econ:crosscorr:InputTooLarge' , ' Number of lags ''nLags'' must not exceed ''Series'' length - 1.');
   end
else
...
if (nargin >= 4) && ~isempty(nSTDs)
   if numel(nSTDs) > 1
      error('econ:crosscorr:NonScalarSTDs' , ' Number of standard deviations ''nSTDs'' must be a scalar.');
   end
   if nSTDs < 0
      error('econ:crosscorr:NegativeSTDs' , ' Number of standard deviations ''nSTDs'' must be non-negative.');
   end
else
