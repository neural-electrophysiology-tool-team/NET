		 ...
		 int nrhs, const mxArray *prhs[])
{
  ...
  ncpus = 1;//sysconf(_SC_NPROCESSORS_ONLN);
  if (ncpus > 4)
  ...
  if (mxIsDouble(prhs[0]) && mxIsDouble(prhs[1]))
    sqrdist_generic(x,y,N,d,q,distmtrx);//sqrdist_thread_launcher<double>(x,y,N,d,q,distmtrx,nthreads);
  else if (mxIsSingle(prhs[0]) && mxIsSingle(prhs[1]))
      sqrdist_generic(x,y,N,d,q,distmtrx);//sqrdist_thread_launcher<float>((float *)x,(float *)y,N,d,q,distmtrx,nthreads);
  else
