...
ops.nSkipCov            = 10;       % not sure about this. % compute whitening matrix from every N-th batch (1)
...
ops.NchanNear   = 21; % default was 32; defined several times in the code; for MEA use 21: gives  5x5 square of electrodes around centre electrode in all directions
ops.Nnearest    = 21;
%%
if nargin~=0
...
ops.nt0     = floor(round(24*ops.fs/1e4)/2)*2+1; % number of samples for the template
ops.nt0min  = floor(round(8*ops.fs/1e4)/2)*2; % where the spikes get aligned 

ops.loc_range       = [round(1.5*ops.fs/1e4)  1];  % ranges to detect peaks; plus/minus in time and channel ([3 1])		
ops.long_range      = [round(10*ops.fs/1e4) 1]; % ranges to detect isolated peaks (default: [30 6]); depends on sampling rate; now: [20 1]		
