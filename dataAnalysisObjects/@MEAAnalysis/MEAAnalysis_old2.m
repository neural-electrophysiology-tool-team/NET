classdef MEAAnalysis < recAnalysis
    ...
    methods
        ...
        function obj=getKiloSorting(obj,varargin)
            ...
            ops=makeConfigKiloSort(obj.currentDataObj.recordingDir,nCh,...
            ...
            %% Save file function (for parallel saves)
            function saveme(varargin)
                leng = size(varargin);
                leng = leng(1);
                if leng==4
                    rez1 = varargin{2};
                    DATA1 = varargin{3};
                    uproj1 = varargin{4};
                    save(varargin{1},'rez1','DATA1','uproj1');
                end
                if leng==3
                    rez1 = varargin{2};
                    DATA1 = varargin{3};
                    save(varargin{1},'rez1','DATA1');
                end
                if leng==2
                    rez1 = varargin{2};
                    save(varargin{1},'rez1');
                end
            end
            %%
            tic; %this is important otherwise fitTemplates crashes since it includes a toc without a tic
            [rez, DATA, uproj] = preprocessData(ops); % preprocess data and extract spikes for initialization
            saveme(resultsFileName,rez, DATA, uproj);
            
            rez                = fitTemplates(rez, DATA, uproj);  % fit templates iteratively
            saveme(resultsFileName,rez, DATA);
            
            rez                = fullMPMU(rez, DATA);% extract final spike times (overlapping extraction)
            saveme(resultsFileName,rez);
            
            rez                = merge_posthoc2(rez); %merge spikes
            saveme(resultsFileName,rez);
