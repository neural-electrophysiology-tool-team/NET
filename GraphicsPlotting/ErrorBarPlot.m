function handles = ErrorBarPlot(barvalues, errors, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend)

%
% Usage: handles = ErrorBarPlot(barvalues, errors, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend)
%
% Ex: handles = ErrorBar(my_barvalues, my_errors, [], [], [], [], [], bone, [], bw_legend)
%
% ErrorBar is the m-by-n matrix of barvalues to be plotted.
% ErrorBar calls the MATLAB bar function and plots m groups of n bars using the width and bw_colormap parameters.
...
% ErrorBar then calls the MATLAB errorbar function to draw barvalues with error bars of length error.
...
if nargin < 10
	error({'Must enter all arguments:','ErrorBar(barvalues, errors, width, groupnames, bw_title, bw_xlabel, bw_ylabel, bw_colormap, gridstatus, bw_legend)','Use [] when for defaut value'});
end
if isempty(barvalues) || isempty(errors)
    error('Bar values and Bar errors must be entered');
end
if isempty(width)
    width=1;
end
