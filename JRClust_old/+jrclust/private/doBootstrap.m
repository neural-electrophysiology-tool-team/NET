function hCfg = doBootstrap(varargin)
    ...
    % load metafile
    if ~isempty(metafile)
        temp = strsplit(metafile{1},'//');
        SMeta_ = jrclust.utils.loadMetadata(strrep(metafile{1},'.bin','.meta'));
...
%                          'probe_file', fullfile(jrclust.utils.basedir(), 'probes', sprintf('%s.prb', SMeta_.probe)));
        C = strsplit(temp{1},'/');
        prbfile = join(C(1:end-1),'/');
        prbfile = strrep(fullfile(prbfile, sprintf('%s.prb', SMeta_.vcProbe)),'.prb','_JRC.prb');
        cfgData = struct('sampleRate', SMeta_.sRateHz, ...
                         'nChans', SMeta_.nChans, ...
                         'bitScaling', SMeta_.scale, ...
                         'headerOffset', 0, ...
                         'dataType', SMeta_.vcDataType, ...
                         'probe_file', prbfile); %strrep(fullfile(temp{1}, sprintf('%s.prb', SMeta_.vcProbe)),'.prb','_JRC.prb'));
