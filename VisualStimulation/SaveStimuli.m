...
%     savepath = strrep(char(strcat(stimSavePath,filesep,obj.user,filesep)), '\','\\');
    ...
    NSKToolBoxMainDir=fileparts(which('identifierOfMainDir4NSKToolBox'));
    configFile=[NSKToolBoxMainDir filesep 'PCspecificFiles' filesep 'stimSavePath.txt']; %JSON encoded
    if exist(configFile,'file')
        fid=fopen(configFile);
        savePathTxt=fscanf(fid,'%s');
        fclose(fid);
    end        
            
    savepath =  [savePathTxt,filesep,obj.user,filesep];
else
    stimSavePath= strrep(obj.stimSavePath,'\', filesep);
...
filename = [savepath,stimname,'_',datestr(now,'mm_dd_yyyy_HHMM'),'.mat'];
