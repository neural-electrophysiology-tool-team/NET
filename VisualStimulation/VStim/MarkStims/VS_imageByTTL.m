classdef VS_imageByTTL < VStim
    ...
    methods
        ...
        function obj=run(obj)
            ...
            i = 0;
            while 1==1
                data=io64(obj.io.ioObj,obj.parallelPortNum+1);
                if data==127    %TTL is 1
                    disp('1');
                     i = i+1;                

%                 Update image buffer for the second time
                for j=1:nScreens
                    Screen('DrawTexture',obj.PTB_win(obj.selectedScreen(j)),obj.imgTex(obj.imgSequence(obj.order(1+i)),j),[],obj.visualFieldRect,obj.rotation);
                end
                obj.applyBackgound;  %set background mask and finalize drawing (drawing finished)

%                 WaitSecs(obj.preSessionDelay); %pre session wait time -- yuval said delete

%                 for i=1:obj.nTotTrials  
                    for j=1:nScreens
                        [obj.on_Flip(1),obj.on_Stim(1),obj.on_FlipEnd(1),obj.on_Miss(1)]=Screen('Flip',obj.PTB_win(obj.selectedScreen(j)));
                    end
                    ...
                    if ~noTimeGapBetweenImages %if there is a black screen between consecutive images
                        ...
                        obj.applyBackgound;  %set background mask and finalize drawing (drawing finished)
                        for j=1:nScreens
                            [obj.off_Flip(1),obj.off_Stim(1),obj.off_FlipEnd(1),obj.off_Miss(1)]=Screen('Flip',obj.PTB_win(obj.selectedScreen(j)),obj.on_Flip(1)+obj.actualStimDuration(obj.selectedScreen(j))-0.5*obj.ifi(obj.selectedScreen(j)));
                        end
                        WaitSecs(0.1);
                        obj.sendTTL(2,false); %session start trigger (also triggers the recording start)
                    elseif 1<obj.nTotTrials
                        obj.off_Flip(1)=obj.on_Flip(1);
                        obj.sendTTL(2,false); %session start trigger (also triggers the recording start)
                    else
                        for j=1:nScreens
                            Screen('FillOval',obj.PTB_win(obj.selectedScreen(j)),obj.visualFieldBackgroundLuminance,obj.visualFieldRect);
                        ...
                        for j=1:nScreens
                            [obj.on_Flip(1+1),obj.on_Stim(1+1),obj.on_FlipEnd(1+1),obj.on_Miss(1+1)]=Screen('Flip',obj.PTB_win(obj.selectedScreen(j)),obj.on_Flip(1)+obj.actualStimDuration(obj.selectedScreen(j))-0.5*obj.ifi(obj.selectedScreen(j)));
                        end
                        obj.sendTTL(2,false); %session start trigger (also triggers the recording start)
                        obj.off_Flip(1+1)=obj.on_Flip(1+1);
                    end
...
% 
                    ...
                    if keyCode(obj.escapeKeyCode)
                        obj.lastExcecutedTrial=1;
                        return;
                    end

                    WaitSecs(wait2NextFrame-(GetSecs-obj.off_Flip(1)));
              
                obj.order=obj.order(1:end-1); %remove the last (never shown stimulus from the list)
               
                elseif data==255  %TTL is 0 
                    disp('0');
        ...
        %class constractor
        function obj=VS_imageByTTL(w,h)
