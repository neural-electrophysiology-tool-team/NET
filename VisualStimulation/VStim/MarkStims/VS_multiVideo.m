...
%             end
            ...
            if obj.playAsImgSequence
                ...
                obj.flip=cell(1,obj.nTotTrials); %nan(obj.nTotTrials,obj.initialFrozenFrames+obj.movFrameCount(1));
                obj.stim=cell(1,obj.nTotTrials); %nan(obj.nTotTrials,obj.initialFrozenFrames+obj.movFrameCount(1));
                obj.flipEnd=cell(1,obj.nTotTrials); %nan(obj.nTotTrials,obj.initialFrozenFrames+obj.movFrameCount(1));
                obj.miss=cell(1,obj.nTotTrials); %nan(obj.nTotTrials,obj.initialFrozenFrames+obj.movFrameCount(1));
%                 tFrame=( 0 : ((obj.skipFrames+1)*obj.ifi(1)) : (( (obj.skipFrames+1)*obj.ifi(1))*(objinitialFrozenFrames+obj.movFrameCount(1)-1)) )';
            ...
            for i=1:obj.nTotTrials
                flip = []; stim = []; flipEnd = []; miss = [];
                currMovie=obj.movieSequence(i);
                obj.sendTTL(1,false);
                obj.sendTTL(1,true); %obj.sendTTL(2,true); %session start trigger (also triggers the recording start)
                
                if ~obj.playAsImgSequence % display longer movies
                    ...
                    [movie,dur,fps,width,height]=Screen('OpenMovie',obj.PTB_win(screenOrder(i)),path_movie{currMovie});
                    % play movie
...
%                         tFlip=Screen('Flip',obj.PTB_win(screenOrder(i)));
                        [flip(frameCounter),stim(frameCounter),flipEnd(frameCounter),miss(frameCounter)] = Screen('Flip',obj.PTB_win(screenOrder(i)));
                        obj.sendTTL(2,true);
                        t = [t flipEnd(frameCounter)];
...
%             for n=1:obj.nVideos
                ...
                if iscell(obj.movieFileName)
                    ...
                    temp = obj.movieFileName;
                    obj.movieFileName = cell(1,1);
                    obj.movieFileName{1} = temp;
                end
...
%             end
        ...
        function obj=calculateVideoTextures(obj,event,metaProp)
            ...
            if nFiles>0
                ...
                if nFiles>1 && obj.playAsImgSequence %single frame mode
                    ...
                    if obj.playAsImgSequence
                        ...
                        sc=1; %for sc = 1:obj.nPTBScreens
                            for ms = 1:obj.nVideos
                                mvFileName = obj.movieFileName{ms};
                                [ moviePtr, duration, fps, width, height, count, aspectRatio ] =  Screen('OpenMovie',obj.PTB_win(sc),[obj.movPathName mvFileName]);
                                obj.movPtr(sc,ms,:) = moviePtr;
                                obj.movDuration(sc,ms,:) = duration;
                                obj.movFps(sc,ms,:) = fps;
                                obj.movWidth(sc,ms,:) = width;
                                obj.movHeight(sc,ms,:) = height;
                                obj.movFrameCount(sc,ms,:) = count;
                                obj.movAspectRatio(sc,ms,:) = aspectRatio;
                            end
                        %end
                    end
                end
                fprintf('\n');
                disp(['Textures are not loaded in memory, info about movies are saved in object ' ]);
%                 disp(['Done preparing movie ' mat2cell(obj.movFrameCount,ones(1,size(obj.movFrameCount,1)))' ' textures']);
        ...
        %class constractor
        function obj=VS_multiVideo(w,h)
